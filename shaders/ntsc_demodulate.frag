...
void main(void)
{
    ...
    const float iq_carrier = fs * NTSC_IQ_CARRIER;
    const float iq_bandwidth = fs * NTSC_IQ_BANDWIDTH;
    const vec2 osc = qam_osc(fs, iq_carrier, n);

    const float fir_n = n + float(NTSC_FIR_SIZE / 2);
    const float fir_bp_0 = iq_carrier - iq_bandwidth;
    const float fir_bp_1 = iq_carrier + iq_bandwidth;
    ...
    for(int i = 0; i < NTSC_FIR_SIZE; ++i) {
        ...
        float lp = fir_lowpass(fs, y_cutoff, NTSC_FIR_SIZE, idx);
        float bp = fir_bandpass(fs, fir_bp_0, fir_bp_1, NTSC_FIR_SIZE, idx);
        float texel = texelFetch(signal, ivec2(fir_n - i, pixcoord.y), 0).r;
        accum_y += lp * texel;
        accum_iq += bp * texel;
    }

    target.x = accum_y;
    target.yz = complex_multiply(osc, vec2(accum_iq, 0.0));
    target.w = 1.0;
}
*/